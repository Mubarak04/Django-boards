...
class PostUpdateViewTestCase(TestCase):
	def setUp(self):
		...
		self.username = 'mubarak'
		self.password = '123'
		user = User.objects.create_user(username=self.username, email='tjmubarak04@gmail.com', password=self.password)
		self.topic = Topic.objects.create(subject='hello world!', board=self.board, starter=user)
		self.post = Post.objects.create(message='Lorem ipsim sit dolor amet', topic=self.topic, created_by=user)
...
class UnauthorizedPostUpdateViewTests(PostUpdateViewTestCase):
	def setUp(self):
		...
		username = 'john'
		password = '321'
		user = User.objects.create_user(username=username, email='tjmubarak04@gmail.com', password=password)
